 ...
 */
	...
	public void print()
	{
		...
		for(int x=0; x<100;x++)						//iterates through the first 100 words
		{
			...
			System.out.println(location.getValue()+
					"  x"+location.getCount());		//this will print the current word and format the count to be placed right beside it
	...
	{
		/* This method returns the number of word checks
		 * the program has done simply by returning the
		 * long value of the variable "checks"
		 */	
		return checks;
	}
	public long getRefChanges()
	{
		/* This method returns the number of reference changes
		 * the program has done simply by returning the long
		 * value of the variable "refChanCount"
		 */
		return refChanCount;
	}
}
